...
export default function Home() {
  ...
  useEffect(() => {
    const getStudents = () => {
      axios
        .get("http://localhost:8070/student/get") // Changed to localhost URL
        .then((res) => {
  ...
  const deleteUser = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://localhost:8070/student/delete/${id}`) // Changed to localhost URL
          .then((res) => {
  ...
  return (
    <div className="text-center mb-4">
      <h5 style={{ textAlign: "center", padding: "3rem" }}>
        Students Attendance Sekolah Tinggi Teknologi Informatika Sony Sugema
      </h5>
      <Link to="/add-student">
        <div className="col-4">
